...
////                    orElseThrow(()->new IllegalArgumentException("term with id not found"+ termId));
////
...
//            Term activeTerm = termRepository.findByStartMonthLessThanEqualAndEndMonthGreaterThanEqualAndSchool_Id(currentDate, currentDate,currentSchoolId)
//                    .orElseThrow(() -> new RuntimeException("No active term found for the current date"));
////
//
//            // Create and populate MpesaSTKTransaction
//            AtomicReference<MpesaSTKTransactions> transaction = new AtomicReference<>(new MpesaSTKTransactions());
//            Student student = mpesaStudentRepository
//                    .findById(accountReference)
//                    .orElseThrow(() -> new IllegalArgumentException("Student not found with admission number: " + accountReference));
...
//            AtomicReference<FeePayments> feePayment = new AtomicReference<>(new FeePayments());
//
...
//                    .orElseThrow(() -> new EntityNotFoundException("School not found with ID: " + currentSchoolId));
...
//            Optional<StudentPaymentHistory> studentPaymentHistoryOpt = studentPaymentHistoryRepository.findByStudentAndTermAndGrade(student, activeTerm, student.getGrade());
//
...
//                BigDecimal totalPaid = transactionHistoryRepository.getTotalPaidAmountByStudentAndTermAndGrade(student.getId(), activeTerm.getId(),student.getGrade().getId());
//
...
//                auditService.logAction("POST", "SACCO TRANSACTIONS ", studentPaymentHistory, "Student payment history updated successfully.");
//            } else {
//                // Handle the case where no payment history exists for the student and term
//                log.warn("No existing StudentPaymentHistory found for student {} in term {}", student.getId(), activeTerm.getId());
//            }
//
//
//
...
public MpesaPaymentResponseDTO initiateSTKPush(@NotNull String phoneNumber, @NotNull Double amount, Long accountReference) {
    ...
    try {
        ...
        payment.setStudentName(student.getFirstName().concat(student.getLastName())); // Assuming Student has getName()
        ...
        payment.setDescription("M-Pesa STK Push Payment - MerchantRequestID: " + responseDTO.getMerchantRequestID());
...
//            auditService.logAction("POST", "FEE PAYMENT", payment, "Fee payment posted successfully");
        } catch (Exception e) {
...
//            auditService.logAction("POST", "FINANCE", finance, "Finance record updated successfully");
        } catch (Exception e) {
...
//                    student[0] = payment.getAccountReference(); // Or however you get the Student from payment
//
//                    if (Integer.parseInt(finalResultCode) == 0) {
//                        // Process SUCCESS
//                        log.info("Processing successful payment for MerchantRequestID: {}", finalMerchantRequestId);
//
//                        // Update the FinanceTransaction with callback data
//                        payment.setAmount(BigDecimal.valueOf(finalAmount));
//                        payment.setReference(finalMpesaCode); // M-Pesa receipt number becomes the reference
...
//                        // Note: You may need to add resultCode field to FinanceTransaction or use description
//                        payment.setUpdatedAt(LocalDateTime.now());
//
//                        this.feePaymentRepository.save(payment);
////                        auditService.logAction("UPDATE", "FEE PAYMENT", payment, "Payment callback processed successfully");
...
//
//                        financeRepository.save(finance);
////                        auditService.logAction("UPDATE", "FINANCE", finance, "Finance record updated after successful payment");
...
//                    } else {
//                        // Process FAILURE
//                        log.info("Processing failed payment for MerchantRequestID: {}", finalMerchantRequestId);
//
//                        // Update payment record with failure information
//                        payment.setDescription("Failed: " + finalResultDesc + " (Code: " + finalResultCode + ")");
//                        payment.setUpdatedAt(LocalDateTime.now());
//
...
//
//                        this.feePaymentRepository.save(payment);
//                        auditService.logAction("UPDATE", "FEE PAYMENT", payment, "Failed payment recorded");
...
//
//            String requestJson = jsonArray.put(jsonObject).toString().replaceAll("[\\[\\]]", "");
//            MediaType mediaType = MediaType.parse("application/json");
